...
int main(int argc,char** argv){
   ...
   int shm_dsc = shmget(IPC_PRIVATE,buff_size,IPC_CREAT | IPC_EXCL | 0600);  //создания области разделяемой памяти с определенным ключом
    if(shm_dsc == -1){
        printf("Error: can't create shared memory\n");
        return 0;
    }
    buffer =(int*) shmat(shm_dsc,NULL,0); //включить область разделяемой памяти в адресное пространство текущего процесса
    pid_t pid = fork();
    if(pid==0){
    ...
    wait(NULL);
    shmctl(shm_dsc, IPC_RMID, NULL); //функция для контроля над сегментами
    return 0;
}
